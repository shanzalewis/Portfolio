 ...
 * This is the EURO class for Vending Machine Simulator which stores the denominations
 * and their quantities for the EURO currency. The user is prompted to insert payment until
...
public class EURO 
{
	...
	EURO() throws Exception      //empty constructor
	{
     ...
     * @param	double total
     */
	EURO(int money, double total_bottle, double total_bag, double total_paper_wrapper, double owed, String[] line, String info, String[] s, Bottles bottle_item, String name, String cost,
			...
			int quantity_remaining, String quant_rem, int new_quantity, double change, Bags bag_item, PaperWrapper paper_wrapper_item, double total) throws Exception
    {
		if (total_bottle > 0)
		{
			owed = total_bottle;               //checks if bottle class returned value
			
		} else if (total_bag > 0) {
			owed = total_bag;                  //checks if bag class returned value
			
		} else if (total_paper_wrapper > 0) {
			owed = total_paper_wrapper;        //checks if paper wrapper class returned value
		}
		
		System.out.println("Pay: $" + owed);   //print amount remaining to pay
		
		while (owed > 0)                      //loop continues as long as there payment needs to be made
		{
			System.out.print( "1 - $0.05, 2 - $0.10, 3 - $0.25, 4 - $1.00, 5 - $5.00, 6 - $10.00, 7 - $20.00" + "\nEnter payment: ");
			money = input.nextInt();           //user selects amount to enter
			if ( (money >=1) || (money <= 7))  //switches between amount selected and subtracts amount from remaining owed
			{
				switch(money) 
				{
				    case 1:
				...
				System.out.println("Please enter a valid option.");  //catches entries outside valid selection and prompts user to make another selection
				break;
			}
			
		} while (owed < 0) {     //loops to calculate change owed to user
			owed *= -1;          
			change = owed;
			calculateChange(money, total, owed, line, info, s, bottle_item, name, cost, amount, item_cost, item_amount, amt_rem, quant_rem, item_num, 
					amount_remaining, change, denom, quantity, currency_quantity, quantity_remaining, quant_rem, new_quantity, total_bottle, total_bag, 
					total_paper_wrapper, bag_item, paper_wrapper_item, quant_rem, quant_rem);
		} while ((-0.01 <= owed) || (owed <= 0.01))	{      //transaction ends when amount owed is zero
			System.out.println();
			...
			String info, String[] line, int new_quantity ) throws Exception
	{
		try
		{
			File f = new File("EURO.txt");
			File fn = new File("EURO2.txt");
  		  ...
  		  Bags bag_item, PaperWrapper paper_wrapper_item, String bags, String paper_wrapper) throws Exception
	{
		...
		if (change > 0)   //checks if user is owed change, switches between highest denomination to distribute, subtracts from change remaining
		{
			if (change >= 20.00)  
			{
				money = 7;
				countEURO(money, total, denom, s, quantity, currency_quantity, quantity_remaining, quant_rem, info, line, new_quantity, owed, bottle_item, name, cost,
     ...
     * @param	double total
     */
	public static void countEURO(int money, double total, String denom, String[] s, String quantity, int currency_quantity, 
